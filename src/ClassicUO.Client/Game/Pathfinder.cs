...
namespace ClassicUO.Game
{
    internal sealed class Pathfinder
    {
        ...
        private bool CreateItemList(List<PathObject> list, int x, int y, int stepState)
        {
            ...
            bool avoidMobiles = ProfileManager.CurrentProfile.AutoAvoidMobiles;
            bool ignoreGameCharacters = !avoidMobiles || ProfileManager.CurrentProfile.IgnoreStaminaCheck || stepState == (int) PATH_STEP_STATE.PSS_DEAD_OR_GM || _world.Player.IgnoreCharacters || !(_world.Player.Stamina < _world.Player.StaminaMax && _world.Map.Index == 0);
